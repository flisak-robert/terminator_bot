...
def location_choice(update, context):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    query.edit_message_text(text="Choose the location of the appointment:", reply_markup=reply_markup)
...
def get_user_name(update,context):
    ...
    update.message.reply_text('Please type your e-mail address')
    return FOURTH
...
def get_user_email(update,context):
    ...
    #Debug MSG
    print(email)
    update.message.reply_text('Please type your phone number')
    return FIFTH
...
def submit_data(update,context):
    ...
    if confirmed != 'https://service.berlin.de/terminvereinbarung/termin/confirm/':
        query.message.reply_text(text='Something went wrong. Please type /start command and try booking the appointment again.')
    else:
        query.message.reply_text(text='Your appointment was booked successfully. Please check your e-mail inbox to get the appointment number and all the information. Enjoy your visit at Bürgeramt!')
